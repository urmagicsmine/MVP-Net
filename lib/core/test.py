...
def get_input_image(image_tensor, center_idx):
    """ Get the input image for the center_idx slice. """
    depth, height, width = image_tensor.shape
    prev_ind = max(0, center_idx - 1)
    cur_ind = center_idx
    next_ind = min(depth, center_idx + 1)
    return cv2.merge([image_tensor[prev_ind, : , :], image_tensor[cur_ind, :, :],
                      image_tensor[next_ind, :, :]])


def ct_detect_all(model, image_tensor, mask_tensor, slice_expand=10, box_proposals=None):
    """Inference each slice within the lung area of that CT.
    """
    if mask_tensor.dtype == numpy.bool:
        mask_tensor = numpy.uint8(mask_tensor) * 255
    mask_index = np.where(mask_tensor == 255)
    z_min, y_min, x_min = [np.min(idx) for idx in mask_idx]
    z_max, y_max, x_max = [np.max(idx) for idx in mask_idx]
    depth, height, width = image_tensor.shape

    z_start = max(0, z_min - outer_interval)
    z_end = min(depth, z_max + outer_interval + 1)

    results_dict = {}
    for z_idx in range(z_start, z_end):
        im = get_input_image(image_tensor, z_idx)
        ...
        results_dict[z_idx] = (score, boxes, cls_boxes)
    num_slice = len(results_dict.keys())

    return results_dict, num_slice 
...
def im_detect_all(model, im, box_proposals=None, timers=None):
    ...
    if cfg.MODEL.MASK_ON and boxes.shape[0] > 0:
        ...
        if isinstance(im, list):
            cls_segms = segm_results(cls_boxes, masks, boxes, im[0].shape[0], im[0].shape[1])
        else:
    ...
    if cfg.TEST.MAM_MASK_BBOX and cls_segms is not None:
        #print(cls_boxes)
        cls_boxes = mask_to_bbox(cls_boxes, cls_segms)
        #print(cls_boxes)
    if cfg.LESION.USE_POSITION:
        return cls_boxes, cls_segms, cls_keyps, return_dict
    else:
        return cls_boxes, cls_segms, cls_keyps


def mask_to_bbox(cls_boxes, cls_segms):

    def mask2bbox(segms, bbox):
        x1, y1 = np.min(segms, axis = 0)
        x2, y2 = np.max(segms, axis = 0)
        x1 = min(max(bbox[0], x1), bbox[2])
        y1 = min(max(bbox[1], y1), bbox[3])
        x2 = min(max(bbox[0], x2), bbox[2])
        y2 = min(max(bbox[1], y2), bbox[3])
        return [x1,y1,x2,y2]

    new_cls_boxes = [[],[]]
    box_array = copy.deepcopy(cls_boxes[1])
    for idx, segm in enumerate(cls_segms[1]):
        if segm == []:
            box_array[idx,:] = [0.0,0.0,0.0,0.0,box_array[idx,4]]
        else:
            tmp = mask2bbox(segm, box_array[idx,:])
            tmp.append(box_array[idx,4])
            box_array[idx,:] = tmp
    new_cls_boxes[1] = box_array

    return new_cls_boxes
...
def segm_results(cls_boxes, masks, ref_boxes, im_h, im_w):
    ...
    for j in range(1, num_classes):
        ...
        for _ in range(cls_boxes[j].shape[0]):
            ...
            contours = find_contours(im_mask, 0.5)
            if len(contours) > 0:
                segms.append(contours[0][:, [1, 0]]) # change (y,x) to (x,y)
            else:
                segms.append([])
...
def _get_blobs(im, rois, target_scale, target_max_size):
    ...
    # Note that during inference, images are not padded with zeros
