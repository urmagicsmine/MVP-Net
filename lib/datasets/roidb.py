...
def combined_roidb_for_training(dataset_names, proposal_files):
    ...
    def get_roidb(dataset_name, proposal_file):
        if cfg.DATA_SOURCE == 'coco':
            ...
            ds = MammoDataset(dataset_name)
        #elif cfg.DATA_SOURCE == 'lesion':
        #    ds = JsonDataset(dataset_name)
            #ds = LesionDataset(dataset_name)
        ...
        if cfg.TRAIN.USE_ADJACENT_LAYER:
            logger.info('Appending adjacent-layer training examples...')
            extend_with_adjacent_entries(roidb, ds)
        if cfg.TRAIN.USE_FLIPPED:
            logger.info('Appending horizontally-flipped training examples...')
            extend_with_flipped_entries(roidb, ds)
        if cfg.TRAIN.USE_Z_FLIPPED:
            logger.info('Appending z_axis-flipped training examples...')
            extend_with_z_flipped_entries(roidb, ds)
...
def extend_with_adjacent_entries(roidb, dataset):
...
def extend_with_z_flipped_entries(roidb, dataset):
    """Flip each entry in the given roidb and return a new roidb that is the
    concatenation of the original roidb and the flipped entries.
...
def filter_for_training(roidb):
    ...
    def is_valid(entry):
        ...
        if cfg.DATA_SOURCE == 'coco':
            ...
            seg_areas = entry['seg_areas']
            count = np.sum(1 * (seg_areas > 0)) 
            valid = (len(fg_inds) > 0 or len(bg_inds)) > 0 and count > 0
