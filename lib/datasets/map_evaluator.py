...
parser.add_argument('--output_dir', nargs='+', type=str, help="txt file folder.")
args = parser.parse_args()

output_folder = args.output_dir[0]
...
print(output_folder)
ground_truth_files_list = glob.glob(osp.join(output_folder, 'ground-truth/*.txt'))
...
for txt_file in ground_truth_files_list:
  ...
  if not os.path.exists(osp.join(output_folder, 'predicted/' + file_id + ".txt")):
...
predicted_files_list = glob.glob(osp.join(output_folder, 'predicted/*.txt'))
...
for class_index, class_name in enumerate(gt_classes):
  ...
  for txt_file in predicted_files_list:
    ...
    file_id = os.path.basename(os.path.normpath(file_id))
    if class_index == 0:
      if not os.path.exists(osp.join(output_folder, 'ground-truth/' + file_id + ".txt")):
...
with open(results_files_path + "/results.txt", 'w') as results_file:
  ...
  for class_index, class_name in enumerate(gt_classes):
    ...
    print('recall:   ', ['%.2f'%i for i in rec[::80]])
    print('precision:', ['%.2f'%i for i in prec[::80]])
    sum_AP += ap
