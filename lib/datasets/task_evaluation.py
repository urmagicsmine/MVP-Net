...
import datasets.mammo_dataset_evaluator as mammo_dataset_evaluator
import datasets.lesion_dataset_evaluator as lesion_dataset_evaluator
import datasets.lungdet_dataset_evaluator as lungdet_dataset_evaluator
...
def evaluate_boxes(dataset, all_boxes, output_dir, use_matlab=False):
    ...
    if _use_mammo_evaluator(dataset):
        coco_eval = mammo_dataset_evaluator.evaluate_boxes(
            dataset, all_boxes, output_dir, use_salt=not_comp, cleanup=not_comp
        )
        box_results = _coco_eval_to_box_results(coco_eval)
    elif _use_lesion_evaluator(dataset):
        coco_eval = json_dataset_evaluator.evaluate_boxes(
            dataset, all_boxes, output_dir, use_salt=not_comp, cleanup=not_comp
        )
        lesion_dataset_evaluator.evaluate_boxes(
            dataset, all_boxes, output_dir, use_salt=not_comp, cleanup=not_comp
            )
        box_results = _coco_eval_to_box_results(coco_eval)
    elif _use_lungdet_evaluator(dataset):
        coco_eval = lungdet_dataset_evaluator.evaluate_boxes(
            dataset, all_boxes, output_dir, use_salt=not_comp, cleanup=not_comp
        )
        box_results = _coco_eval_to_box_results(coco_eval)
...
def evaluate_masks(dataset, all_boxes, all_segms, output_dir):
    ...
    not_comp = not cfg.TEST.COMPETITION_MODE
    if _use_mammo_evaluator(dataset):
        coco_eval = mammo_dataset_evaluator.evaluate_masks(
            dataset, all_boxes, all_segms, output_dir, use_salt=not_comp, cleanup=not_comp
        )
        mask_results = _coco_eval_to_box_results(coco_eval)
...
def _use_mammo_evaluator(dataset):
    return dataset.name.find('ddsm') > -1 or dataset.name.find('inbreast') > -1 \
            or dataset.name.find('domestic') > -1 or dataset.name.find('gansu') > -1 or dataset.name.find('nanjing') > -1 \
            or dataset.name.find('by31') > -1 or dataset.name.find('zhongri') > -1 or dataset.name.find('shougang') > -1 \
            or dataset.name.find('hologicmix1') > -1

def _use_lung_evaluator(dataset):
    return dataset.name.find('njjz') > -1

def _use_lesion_evaluator(dataset):
    return dataset.name.find('lesion') > -1 #or dataset.name.find('lungbinary') > -1

def _use_json_dataset_evaluator(dataset):
    """Check if the dataset uses the general json dataset evaluator."""
    return dataset.name.find('coco_') > -1 or cfg.TEST.FORCE_JSON_DATASET_EVAL or \
           dataset.name.find('lungtop3') > -1 or dataset.name.find('lungsup') > -1 \
            or dataset.name.find('lungsub') > -1 or dataset.name.find('lungbinary') > -1   

def _use_lungdet_evaluator(dataset):
    #return dataset.name.find('lungtop3') > -1 \
    #        or dataset.name.find('lungsub') > -1 or
    #return dataset.name.find('lungbinary') > -1
    return False
