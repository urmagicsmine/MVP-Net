...
class Generalized_RCNN(nn.Module):
    def __init__(self):
        ...
        assert (not cfg.MODEL.LR_VIEW_ON) or (not cfg.MODEL.GIF_ON) or (not cfg.MODEL.LRASY_MAHA_ON)
        if cfg.MODEL.LR_VIEW_ON:
            #assert cfg.FPN.FPN_ON is False
            self.lr_view_net = lrview_net_body.get_lrv_net(
                self.Conv_Body.dim_out * 2,
    ...
    def _forward(self, data, im_info, roidb=None, **rpn_kwargs):
        ...
        if not cfg.MODEL.RPN_ONLY:
            ...
            if cfg.LESION.SHALLOW_POSITION:
                return_dict['pos_cls_pred'] = pos_cls_pred
                return_dict['pos_reg_pred'] = pos_reg_pred
            if cfg.LESION.POSITION_RCNN: pos_cls_rcnn, pos_reg_rcnn = self.Position_RCNN(box_feat)
        else:
        ...
        if self.training:
            ...
            # RCNN Position
            if cfg.LESION.POSITION_RCNN:
                pos_cls_rcnn_loss,pos_reg_rcnn_loss,accuracy_position_rcnn = position_rcnn_losses(pos_cls_rcnn,pos_reg_rcnn,roidb)
                return_dict['losses']['RcnnPosCls_loss'] = pos_cls_rcnn_loss
                return_dict['losses']['RcnnPosReg_loss'] = pos_reg_rcnn_loss
                return_dict['metrics']['accuracy_position_rcnn'] = accuracy_position_rcnn
    ...
    def _get_lrview_blob_conv(self, blob_conv):
        """ Get the blob_conv for lrview net. Support three different types"""
        if cfg.MODEL.LR_VIEW_ON or cfg.MODEL.GIF_ON or cfg.MODEL.LRASY_MAHA_ON:
            tmp_conv = []
            if isinstance(blob_conv, list):
                for blob in blob_conv:
                    batch_size = blob.size(0)
                    # the first sample is positive and the second is comparision
                    assert batch_size == 2
                    pos_blob_conv = blob[:1]
                    cmp_blob_conv = blob[1:]
                    if cfg.LR_VIEW.FUSION_POLICY == 'diff':
                        if cfg.MODEL.LR_VIEW_ON:
                            tmp_conv.append(self.lr_view_net(torch.cat([pos_blob_conv - cmp_blob_conv, cmp_blob_conv], 1)))
                        elif cfg.MODEL.LRASY_MAHA_ON:
                            tmp_conv.append(self.lrasy_maha_net(pos_blob_conv, cmp_blob_conv))
                        elif cfg.MODEL.GIF_ON:
                            tmp_conv.append(self.gif_net(pos_blob_conv - cmp_blob_conv, cmp_blob_conv))
                    else:
                        if cfg.MODEL.LR_VIEW_ON:
                            tmp_conv.append(self.lr_view_net(torch.cat([pos_blob_conv, cmp_blob_conv], 1)))
                        elif cfg.MODEL.LRASY_MAHA_ON:
                            tmp_conv.append(self.lrasy_maha_net(pos_blob_conv, cmp_blob_conv))
                        elif cfg.MODEL.GIF_ON:
                            tmp_conv.append(self.gif_net(pos_blob_conv, cmp_blob_conv))
                if len(tmp_conv) == 1:
                ...
                # the first sample is positive and the second is comparision
                assert batch_size == 2
                pos_blob_conv = blob_conv[:1]
                cmp_blob_conv = blob_conv[1:]
                if cfg.LR_VIEW.FUSION_POLICY == 'diff':
                    if cfg.MODEL.LR_VIEW_ON:
                        blob_conv = self.lr_view_net(torch.cat([pos_blob_conv - cmp_blob_conv, cmp_blob_conv], 1))
                    elif cfg.MODEL.LRASY_MAHA_ON:
                        blob_conv = self.lrasy_maha_net(pos_blob_conv, cmp_blob_conv)
                    elif cfg.MODEL.GIF_ON:
                        blob_conv = self.gif_net(pos_blob_conv - cmp_blob_conv, cmp_blob_conv)
                else:
                    if cfg.MODEL.LR_VIEW_ON:
                        blob_conv = self.lr_view_net(torch.cat([pos_blob_conv, cmp_blob_conv], 1))
                    elif cfg.MODEL.LRASY_MAHA_ON:
                        blob_conv = self.lrasy_maha_net(pos_blob_conv, cmp_blob_conv)
                    elif cfg.MODEL.GIF_ON:
                        blob_conv = self.gif_net(pos_blob_conv, cmp_blob_conv)
        return blob_conv
