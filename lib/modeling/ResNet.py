...
def ResNet50_conv5_small_body():
    return ResNet_convX_body((6, 4, 3, 3))
...
class ResNet_convX_body(nn.Module):
    def __init__(self, block_counts):
        ...
        if len(block_counts) == 4:
            ...
            self.res5, dim_in = add_stage(dim_in, 2048, dim_bottleneck * 8, block_counts[3],
                        cfg.RESNETS.RES5_DILATION, stride_init, attention=self.use_attention)
    ...
    def detectron_weight_mapping(self):
        if cfg.RESNETS.USE_GN:
            ...
            # Added by lizihao, 2018/11/28
            # Skip load conv1 pretrained weights when input channel > 3.
            if cfg.MODEL.LOAD_IMAGENET_PRETRAINED_WEIGHTS == True and not cfg.LESION.SLICE_NUM == 3:
                mapping_to_detectron = {}
                orphan_in_detectron = ['conv1_w', 'res_conv1_bn_s', 'res_conv1_bn_b', 'conv1_b', 'fc1000_w', 'fc1000_b']
            else:
                mapping_to_detectron = {
                    'res1.conv1.weight': 'conv1_w',
...
def add_stage(inplanes, outplanes, innerplanes, nblocks, dilation=1, stride_init=2, attention=False):
    ...
    for _ in range(nblocks):
        res_blocks.append(add_residual_block(
            inplanes, outplanes, innerplanes, dilation, stride, attention
        ))
        inplanes = outplanes
        stride = 1
...
def add_residual_block(inplanes, outplanes, innerplanes, dilation, stride, attention=False):
    ...
    res_block = trans_func(
        ...
        downsample=downsample, attention=attention)
