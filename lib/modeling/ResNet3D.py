# Author __gongping__@7.29.18 DeepWise Inc.

# Note that in 3D detection batchnorm3d is trained from scratch
# Thus AffineChannel is deprecated
#
...
def ResNet50_conv3_body():
    return ResNet_convX_body((3, 4))
...
class ResNet_convX_body(nn.Module):
    def __init__(self, block_counts):
        ...
        elif len(block_counts) == 3:
            self.spatial_scale = 1 / 16  # final feature scale wrt. original image scale

        self.dim_out = dim_in
        # Added by shuzhang to initialize Conv3d params with XavierFill
        self._init_weights()
        self._init_modules()

    def _init_weights(self):
        """Added to init Conv3d modules in the ResNet"""
        def init_func(m):
    ...
    def _init_modules(self):
        ...
        #self.apply(lambda m: freeze_params(m) if isinstance(m, nn.BatchNorm3d) else None)
...
class ResNet_roi_conv5_head(nn.Module):
    def __init__(self, dim_in, roi_xform_func, spatial_scale):
        ...
        dim_bottleneck = cfg.RESNETS.NUM_GROUPS * cfg.RESNETS.WIDTH_PER_GROUP
        stride_init = cfg.FAST_RCNN.ROI_XFORM_RESOLUTION // cfg.FAST_RCNN.ROI_RES5_RESOLUTION
        self.res5, self.dim_out = add_stage(dim_in, 2048, dim_bottleneck * 8, 3,
                                            dilation=1, stride_init=stride_init)
        self.avgpool = nn.AvgPool3d(cfg.FAST_RCNN.ROI_RES5_RESOLUTION)
...
def basic_bn_stem():
    ...
    return nn.Sequential(OrderedDict([
        ...
        # ('maxpool', nn.MaxPool3d(kernel_size=3, stride=2, padding=0, ceil_mode=True))]))
...
class bottleneck_transformation(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, innerplanes, stride=1, dilation=1, group=1,
                 downsample=None):
        super().__init__()
        # In original resnet, stride=2 is on 1x1x1.
        # In fb.torch resnet, stride=2 is on 3x3x3.
        stride_3d = (1, stride, stride) if cfg.LESION.NO_DEPTH_PAD else stride
        (str1x1x1, str3x3x3) = (stride_3d, 1) if cfg.RESNETS.STRIDE_1X1 else (1, stride_3d)
        self.stride = stride_3d

        self.conv1 = nn.Conv3d(
            inplanes, innerplanes, kernel_size=1, stride=str1x1x1, bias=False)
        self.bn1 = nn.BatchNorm3d(innerplanes)

        self.conv2 = nn.Conv3d(
            innerplanes, innerplanes, kernel_size=3, stride=str3x3x3, bias=False,
            padding=1 * dilation, dilation=dilation, groups=group)
        self.bn2 = nn.BatchNorm3d(innerplanes)
