...
#from modeling.position_heads.py import position_outputs
...
class fast_rcnn_disentangle_outputs(nn.Module):
    """ Reg and Cls Outputs from disentagnled feature vectors"""
    def __init__(self, dim_in):
        super().__init__()

        # Set dim_in according to the fusion type and feature usage type.
        if cfg.CONTEXT_ROI_POOLING.MERGE_TYPE == 'concat':
            cls_dim_in = dim_in * 2
        else:
            cls_dim_in = dim_in
        if cfg.CONTEXT_ROI_POOLING.MERGE_TYPE == 'concat' and cfg.CONTEXT_ROI_POOLING.SHARE_FEATURE:
            dim_in = dim_in * 2

        self.cls_score = nn.Linear(cls_dim_in, cfg.MODEL.NUM_CLASSES)
        if cfg.MODEL.CLS_AGNOSTIC_BBOX_REG:  # bg and fg
            self.bbox_pred = nn.Linear(dim_in, 4 * 2)
        else:
            self.bbox_pred = nn.Linear(dim_in, 4 * cfg.MODEL.NUM_CLASSES)
    ...
    def forward(self, cls_x, reg_x):
        if cls_x.dim() == 4:
            cls_x = cls_x.squeeze(3).squeeze(2)
        if reg_x.dim() == 4:
            reg_x = reg_x.squeeze(3).squeeze(2)
...
def fast_rcnn_losses(cls_score, bbox_pred, label_int32, bbox_targets,
    ...
    #fg_num = (rois_label==1).data.sum()
    #bg_num = (rois_label==0).data.sum()
...
class roi_2mlp_head(nn.Module):
    ...
    def __init__(self, dim_in, roi_xform_func, spatial_scale):
        ...
        if cfg.LESION.USE_3DCE or cfg.LESION.MULTI_MODALITY and \
                not cfg.LESION.FUSION_BEFORE_RPN and not cfg.LESION.MM_POS:
            self.fc1 = nn.Linear(cfg.LESION.NUM_IMAGES_3DCE * dim_in * roi_size**2, hidden_dim)
        else:
...
class roi_Xconv1fc_gn_head(nn.Module):
    ...
    def __init__(self, dim_in, roi_xform_func, spatial_scale):
        ...
        if cfg.LESION.USE_3DCE or cfg.LESION.MULTI_MODALITY:
            module_list.extend([
                nn.Conv2d(cfg.LESION.NUM_IMAGES_3DCE * dim_in, hidden_dim, 3, 1, 1, bias=False),
        ...
        for i in range(1,cfg.FAST_RCNN.NUM_STACKED_CONVS):
            module_list.extend([
                nn.Conv2d(dim_in, hidden_dim, 3, 1, 1, bias=False),
                nn.GroupNorm(net_utils.get_group_gn(hidden_dim), hidden_dim,
                             eps=cfg.GROUP_NORM.EPSILON),
