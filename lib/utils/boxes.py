...
def aug_align_box(bbox, im_shape, pad=128, shift = 40, aug_ratio=0.5):
    """Used for Mam detection.
       Randomly perturb the bbox for left and right view alignment.
       Param: bbox, list or tuple of (x1,y1,x2,y2)
    """
    #pad = cfg.TRAIN.PADDING
    im_shape = [-pad, -pad, im_shape[0]+pad, im_shape[1]+pad]
    if random.random() <= aug_ratio:
        return [int(min(max(coord + (random.random() - 0.5)* shift, im_shape[idx%2]), im_shape[idx%2 + 2])) for idx, coord in enumerate(bbox)]
    else:
        return bbox
