...
# additional libs for online cropping
import copy
from datasets.roidb import rank_for_training, add_bbox_regression_targets, _compute_and_log_stats
from datasets.online_roidb_cropping import online_cropping_roidb_v2
...
def main():
    ...
    if args.dataset == "coco2017":
        ...
        cfg.TRAIN.DATASETS = ('gansu2_train',)
        cfg.MODEL.NUM_CLASSES = 2
    elif args.dataset == 'gansu3':
        cfg.TRAIN.DATASETS = ('gansu3_train',)
        cfg.MODEL.NUM_CLASSES = 2
    elif args.dataset == 'nanjing1':
        cfg.TRAIN.DATASETS = ('nanjing1_train',)
        cfg.MODEL.NUM_CLASSES = 2
    elif args.dataset == 'gansu_12':
        cfg.TRAIN.DATASETS = ('gansu1_train', 'gansu2_train')
        cfg.MODEL.NUM_CLASSES = 2
    ...
    if not cfg.TRAIN.ONLINE_RANDOM_CROPPING:
    ...
    try:
        ...
        for args.epoch in range(args.start_epoch, args.start_epoch + args.num_epochs):
            ...
            # online generate the cropped roidb for each epoch
            if cfg.TRAIN.ONLINE_RANDOM_CROPPING:
                roidb_tmp  = copy.deepcopy( roidb )
                roidb_tmp = online_cropping_roidb_v2( roidb_tmp )

                if cfg.TRAIN.ASPECT_GROUPING or cfg.TRAIN.ASPECT_CROPPING:
                    ratio_list, ratio_index = rank_for_training(roidb_tmp)
                else:
                    ratio_list, ratio_index = None, None
                add_bbox_regression_targets(roidb_tmp)
                _compute_and_log_stats(roidb_tmp)
